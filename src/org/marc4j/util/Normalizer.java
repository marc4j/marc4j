...
/**
 * This class is a shim interface to handle the normalization of unicode
 * characters. This functionality is available in java 1.6+ and newer, via the
 * java.text.Normalizer class. If for some reason that class is not available,
 * this code will attempt to fall back to using either the class
 * com.ibm.icu.text.Normalizer from the icu4j library or to using the class
 * com.solrmarc.icu.text.Normalizer which is based on (an older version) of the
 * preceding and which is included as a part of SolrMarc. Since Java 1.6 is now
...
public class Normalizer {
    ...
    private static void initializeNormalizer() {
        try {
            ...
            // normalizerInAvailableJar.getMethods();
            normalizeMethod = normalizerInAvailableJar.getMethod("normalize",
                    java.lang.CharSequence.class, normalizerFormClass);
            final Method valueOf = normalizerFormClass.getMethod("valueOf", String.class);
            ...
            try {
                composeMethod = normalizerInAvailableJar.getMethod("compose", String.class,
                        boolean.class);
                decomposeMethod = normalizerInAvailableJar.getMethod("decompose", String.class,
                        boolean.class);
