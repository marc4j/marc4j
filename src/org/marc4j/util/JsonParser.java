...
public class JsonParser extends Object {
	 ...
	 * 
	 * @param opt
	 *            Parsing option flags, combined from OPT_xxx values.
	 */
	 ...
	 * <code>isQuoted()</code> and <code>stripQuotes</code>. Alternatively, the
	 * methods <code>getTypedMemberValue()</code> or
	 * <code>createTypeValue()</code> may be used to get a Java typed object,
	 ...
	 * <li>true - Boolean.TRUE (not case sensitive, unlike strict JSON
	 * specification requirement)
	 * <li>false - Boolean.FALSE (not case sensitive, unlike strict JSON
	 * specification requirement)
	 * <li>null - null (not case sensitive, unlike strict JSON specification
	 * requirement)
	 ...
	 *             If an unquoted value which is no null, true or false is not a
	 ...
	 * 
	 * @param inpnam
	 *            A text description of the source, used only for location text.
	 * @param inpsrc
	 *            Input source.
	 * @param inpcls
	 *            Whether to close the input source at end-of-input.
	 */
	 ...
	 *            Input source.
	 * @param inpenc
	 *            Character encoding used by the input source.
	 * @param inpcls
	 *            Whether to close the input source at end-of-input.
	 * @throws Escape.ENCODING
	 *             If the encoding string is not supported by the JVM.
	 */
	 ...
	 *            Character encoding used by the input source.
	 * @param bufsiz
	 *            Size of input buffer for reading from the file.
	 * @throws Escape.ENCODING
	 *             If the encoding string is not supported by the JVM.
	 * @throws Escape.IOERROR
	 *             If the file could not be found or accessed.
	 */
	public JsonParser setInput(String inpfil, String inpenc, int bufsiz) {
		...
		inpName = inpfil;
		try {
			inpReader = new InputStreamReader(new BufferedInputStream(
		...
		if (optPreloadInput) {
			inpReader = preloadInput(inpfil, inpReader, inpClose, Math.min(
	...
	public JsonParser setInput(File inpfil, String inpenc, int bufsiz) {
		...
		if (optPreloadInput) {
			inpReader = preloadInput(inpfil.toString(), inpReader, inpClose,
					Math.min(Integer.MAX_VALUE, (int) inpfil.length()));
			inpClose = true;
		}
				...
				while ((ich = readChar()) != -1) {
					...
					if (ich == '\\') {
						...
						case 'u': { // unicode 0x0000 - 0xFFFF
							...
							storeChar((char) decodeHexChar((char) ic1,
									(char) ic2, (char) ic3, (char) ic4, lin,
									col));
						}
							continue;
						...
						if (ich < 0x0020
							...
							throw parserError(
									...
									"A quoted literal may not contain any control characters"
											+ (optMultilineStrings ? " except CR and LF"
													: "")
					...
					case '\n': {
						if (accumulator.length() == 0) { // empty value ignored
							mbrName = ""; // continue as if member-value event
					...
					case '}': {
						mbrValue = getAccumulatedText(false);
						if (objectData.arrayDepth == 0 && mbrName.length() == 0
								&& mbrValue.length() > 0) {
							throw parserError(
									Escape.MALFORMED,
									"Object member name or value is missing in a Name:Value pair (this is possibly caused by missing array brackets in an array)");
	...
	private Location createLocation(int lin, int col) {
		return new Location(inpName, lin, col,
				((mbrName != null && mbrName.length() != 0) ? mbrName
						: objectData.arrayName));
	}
	 ...
	 * order to overlap loading and processing of multiple files using a deferred
	 * processing pipeline.
	 */
		...
		if (val != null && (len = val.length()) > 1) {
			...
			if ((ch0 == '"' || ch0 == '\'') && ch0 == val.charAt(len - 1)) {
				return true; // NOTE: Don't test for unescaped quotes because
								// the value has already been unescaped as it
								// was parsed.
			}
		}
		return false;
	...
	static public class Escape extends RuntimeException {
		 ...
		 *            The error code.
		 * @param detail
		 *            Specific detail text indicating the cause of the error.
		 */
		 ...
		 *            Specific detail text indicating the cause of the error.
		 * @param cause
		 *            The causitive throwable object, if any.
		 */
